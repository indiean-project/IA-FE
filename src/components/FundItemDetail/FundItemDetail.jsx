...
function FundItemDetail({ nav, navRef }) {
    ...
    useEffect(() => {
        ...
        if (fund.rewardList != undefined) {
            let amount = new Array(fund.rewardList.length)
    ...
    const onClickReward = (reward, idx, num) => {
        if (num < 1) {
            let count = [...orderAmount];
            count[idx] = orderAmount[idx] + 1;
            let list = [...orderList];
            if (orderList[idx] != undefined) {
                list[idx] = {
                    ...list[idx],
                    amount: count[idx]
                }
            ...
            <div className='fundItemDetail__container'>
                ...
                <div className='fundItemDetail__content'>
                    ...
                    <div className='fundItemDetail__reward'>
                            ...
                            {fund.rewardList != null && fund.rewardList.map((item, idx) => {
                                return (
                                    <div className='reward__option__item' key={item.rewardNo}  >
                                        <PlusSquareFill size={35} onClick={() => onClickReward(item, idx, 1)} />
                        ...
                        <div className='fundItemDetail__reward__item'>
                            {orderList.length != 0 ? orderList.filter(item => item.amount != 0).map((item, idx) => {
                                return (
                                    <div className='reward__option__item' key={item.rewardNo}>
                                        <div className='reward__option__item__amount'>
                                            <input type="number" value={orderAmount[idx]} onChange={(e) => onClickReward(item, idx, e.target.value)} />
                                        </div>
                                        <div className='slash'>/</div>
                                        <div className='reward__option__item__content'>
                                            <div className='reward__option__item__header'>
                                                <div className='reward__title'>{item.rewardName}</div>
                                                <div className='reward__price'>{item.price}원</div>
                                            </div>
                                            <div className='reward__info'>
                                                리워드 간단 설명
                                            </div>
